 ...
 */
		...
		{	
			if(_player_state == PLAYER_STATE_COSMETIC) LeaveCosmeticMode(msg.param); //这时状态可能不对
		}
		return 0;
		...
		case GM_MSG_QUERY_PROPERTY:
		{
			...
			if(data.prayspeed >= 100) data.prayspeed = 90;	//很奇怪，但技能那边就是这样写的
		...
		case GM_MSG_QUERY_PROPERTY_REPLY:
		{
			...
			if(self_data.prayspeed >= 100) self_data.prayspeed = 90;	//很奇怪，但技能那边就是这样写的
...
gplayer_imp::FillAttackMsg(const XID & target,attack_msg & attack,int dec_arrow)
{
	...
	if(!_pvp_enable_flag) attack.force_attack = 0; //如果没有打开PK开关，则关闭强制攻击选项
...
gplayer_imp::FillEnchantMsg(const XID & target, enchant_msg & enchant)
{
	...
	if(!_pvp_enable_flag) enchant.force_attack = 0;//如果没有打开PK开关，则关闭强制攻击选项
...
{
	int _______crit_damage_bonus = _skill.GetPraySpeed();//_imp->_crit_damage_bonus;
	
	int _______damage_reduce = _damage_reduce;//((gplayer*)_parent)->invisible_degree
	int _______magic_damage_reduce = _magic_damage_reduce[0];//((gplayer*)_parent)->anti_invisible_degree
	
	_runner->self_get_property(_basic.status_point,_cur_prop,
								_attack_degree, _defend_degree, 
								_crit_rate+_base_crit_rate, _______crit_damage_bonus, 
								_______damage_reduce, _______magic_damage_reduce,
...
/*
	...
	*/
	...
	{
		check_assist_next_time = cur_time + 2; // set next check time
...
{
	bool is_remote = NPCService::IsRemoteService(service_type);
	
	if (is_remote && service_npc::IsValidPepegaNPC()/* && _provider.id.id == -1*/)
	{
		int pepegaWID = service_npc::GetPepegaNPC();
		//A3DVECTOR curpos();
		
		// SET ELITE NPC ID
		_provider.id.id = pepegaWID; // SAVED_MARADONA_WID
		_provider.pos = _parent->pos;
		
		//case C2S::SERVICE_HELLO:
		{
			XID target;
			MAKE_ID(target,pepegaWID);
			SayHelloToNPC(target);
		}
		//case C2S::SERVICE_GET_CONTENT:
		{
			QueryServiceContent(service_type);
		}
	}
	
	bool can_use = CanUseService();
	printf("[__PEPEGA__][SendServiceRequest] id=%d, can_use=%s, is_remote=%s, _provider.id.id=%d\n", service_type, (can_use?"true":"false"), (is_remote?"true":"false"), _provider.id.id);
	service_executor * executor;
	if( can_use && (executor = service_manager::GetExecutor(service_type)) )
	{
...
gplayer_imp::PlayerUseItem(int where, size_t inv_index, int item_type,size_t count)
{
	...
	
	{
		...
		if (item_type == 59896)
		{
			FactionAllianceCongregateRequest();
			return true;
		}
...
{
	if(_basic.level < MAFIA_PVP_LEVEL_LIMIT) return; // 30级以下玩家不参与
...
{
	if(!world_manager::GetWorldParam().pve_mode) return; //若是PVP模式则不允许手动打开此开关，直接返回
...
{
	if(!world_manager::GetWorldParam().pve_mode) return; //若是PVP模式则不允许手动关闭此开关，直接返回
	if(!_pvp_enable_flag) return;
	if(_pvp_cooldown > 0) 
	{
...
bool gplayer_imp::ActivateSharpener(int id, int equip_idx)
{
	...
	for( ; i<3 && i<sizeof(ess->addon)/sizeof(int); i++)
	{
		...
		if(addon_list[i].arg[1] != 0xFFFF) return false; //必须是时效附加属性	
...
gplayer_imp::SummonPet2(int pet_egg_id, int skill_level, int life_time)
{
	...
	if(petess->id_type != 28752) return false;			//限制必须是召唤物
...
gplayer_imp::SummonPlantPet(int pet_egg_id, int skill_level, int life_time, const XID & target, char force_attack)
{
	...
	PET_ESSENCE * petess = (PET_ESSENCE *)world_manager::GetDataMan().get_data_ptr(ess->id_pet, ID_SPACE_ESSENCE, datatype);
	if(petess == NULL || datatype != DT_PET_ESSENCE)	return false;
	if(petess->id_type != 28913) return false;			//限制必须是植物
...
gplayer_imp::AddPetToSlot(void * arg, int inv_index)
{
	...
	if(data.pet_class == pet_data::PET_CLASS_SUMMON 
			|| data.pet_class == pet_data::PET_CLASS_PLANT) return false;	//召唤物和植物不是这样召唤的
	int index = _petman.AddPetData(data);
	if(index >= 0)
	{
		_runner->gain_pet(index, &data,sizeof(data));
...
gplayer_imp::ConvertPetToEgg(size_t slot_index)
{
	...
	if(rst < 0) return S2C::ERR_SERVICE_UNAVILABLE;		//放入包裹栏失败
	ASSERT(data.count == 0);
...
{
	if(world_manager::GetWorldTag() != 1) return;	//只准从大世界进入
...
{
	if(!world_manager::GetInstance()->IsCountryTerritoryWorld()) return;		//限定必须从国战首都场景进入
...
gplayer_imp::FirstAcquireItem(const item_data* itemdata)
{
	...
	if(CheckGMPrivilege() && !player_template::GetDebugMode()) return;	//正式服中GM获得稀有物品不喊话
	struct 
	{
		int item_id;
...
gplayer_imp::CheckItemBindCondition(size_t index,  int id, int can_webtrade)
{
	...
	if(((it.proc_type & item::ITEM_PROC_NO_BIND_MASK) == item::ITEM_PROC_NO_BIND_MASK ) && !(it.proc_type & item::ITEM_PROC_TYPE_CAN_WEBTRADE)) return false;	//已经绑定过了
	if(((it.proc_type & item::ITEM_PROC_NO_BIND_MASK) == item::ITEM_PROC_NO_BIND_MASK ) && (it.proc_type & item::ITEM_PROC_TYPE_CAN_WEBTRADE) && can_webtrade) return false;	//已经绑定过了
	...
	if((it.proc_type & item::ITEM_PROC_TYPE_BIND) && (it.proc_type & item::ITEM_PROC_TYPE_CAN_WEBTRADE) && can_webtrade) 
		return false;
	if(it.proc_type & item::ITEM_PROC_TYPE_BIND2) return false;	//装备后绑定不让绑
...
gplayer_imp::SendFarCryChat(char channel, const void * msg, size_t size, const void* data, size_t dsize)
{
	...
	//冷却
	SetCoolDown(COOLDOWN_INDEX_FARCRY, 5000);
...
gplayer_imp::ProduceItem3(const recipe_template & rt,int materials[16], int idxs[16], int equip_id, int equip_inv_idx, char inherit_type)
{
	...
	if(inherit_type & PRODUCE_INHERIT_STONE) inherit_type |= PRODUCE_INHERIT_SOCKET;	//如继承宝石必须继承孔数
	if(rt.inherit_fee_factor < 0.000001f) inherit_type |= (PRODUCE_INHERIT_REFINE | PRODUCE_INHERIT_SOCKET | PRODUCE_INHERIT_STONE); //没有继承费用时，强制继承精炼空数宝石
	if(rt.inherit_engrave_fee_factor < 0.000001f) inherit_type |= PRODUCE_INHERIT_ENGRAVE;//没有继承费用时，强制继承镌刻属性
...
gplayer_imp::ProduceItem5(const recipe_template& rt,int materials[16], int idxs[16], int equip_id, int equip_inv_idx, char inherit_type)
{
    ...
    if(rt.inherit_addon_fee_factor < 0.000001f) inherit_type |= PRODUCE_INHERIT_ADDON; //没有继承费用，强制继承附加属性
	if(eq_dt == DT_WEAPON_ESSENCE)
	{
		if(((WEAPON_ESSENCE*)eq_ess)->level > ((WEAPON_ESSENCE*)target_eq_ess)->level) return false;
...
//joslian
bool 
gplayer_imp::FactionAllianceCongregateRequest(int reply_level_req, int reply_sec_level_req, int reply_reincarnation_times_req)
{
	if (world_manager::GetWorldTag() != 1) // World
		return false;
	...
	for (abase::hash_map<int,int>::iterator it = _faction_alliance.begin(); it != _faction_alliance.end(); ++it)
	{
		...
		bool b = GMSV::SendFactionCongregateRequest(fanctionId, _parent->ID.id, &data, sizeof(data));
		if (b)
		{
			result = true;
		}
...
gplayer_imp::GetKnockBackPos(const A3DVECTOR& attacker_pos, float back_distance, A3DVECTOR& back_pos) 
{
	...
	if(step_count < 2) return -1;	//后退距离太小 
...
void gplayer_imp::ElfPlayerInsertExp(unsigned int exp, char use_sp)
{
	...
	bool IS_LEVELUP_OK = false;
	exp = 840000000; // 840 000 000
	...
	/*if(use_sp)
		...
		_basic.exp -= (int)_exp;*/
	SetRefreshState();
	GLog::log(GLOG_INFO,"用户%d用%d点%s给小精灵注经验,现有经验%d,元神%d", _parent->ID.id, 0/*(use_sp?_exp/5:_exp)*/, (use_sp?"元神":"经验"), _basic.exp, _basic.skill_point);
...
bool gplayer_imp::RechargeEquippedElf(size_t index, size_t count)	//与飞剑加耐力逻辑完全相同
{
	if(count == 0 || index >= _inventory.Size()) return false;
	item & element = _inventory[index];
	if(element.type == -1 || element.count < count) return false;
...
gplayer_imp::ChangeEquipAddon(unsigned char equip_idx,unsigned char equip_socket_idx,int old_stone_type,int new_stone_type,int recipe_type,int* materials_ids,unsigned char* idxs,int count)
{
	...
	if(equip_idx >= inv_size || inv[equip_idx].type <= 0 
	   || !inv[equip_idx].HasAddonAtSocket(equip_socket_idx,old_stone_type))// HasAddonAtSocket 顺带判断了 inv[equip_idx].body != NULL 
	{
		//校验该物品是否存在
	...
	else
	{
		...
		for(int idx = 0; idx < count; ++idx)
		{
			...
			if(!inv.IsItemExist(idxs[idx], acrt->material_list[idx].item_id , acrt->material_list[idx].count )
					|| !unique_idx.insert(idx).second ) // 已经检测过materials_id[idx] == ess->materials[idx]
			{
...
void gplayer_imp::PlayerLateSignin(char type,int itempos,int desttime)
{
	...
	{
		if(itempos == -1)  itempos = inv.Find(0, LATE_DAY_SIGNIN_ITEM);   // 移动客户端需要系统找位
...
int gplayer_imp::PlayerTrickBattleApply(int chariot)
{
	...
	if(cfg->pre_chariot > 0) return -1;	//只能选择初级战车
...
void gplayer_imp::SelfChatMessage(const char* _msg, int _msg_size, int _channel) // int _cs_index, int _cs_sid, int _roleid,
{
	gplayer * pPlayer = (gplayer *)_parent;
	
	int _data_size = 0;
	char _data[_data_size];
	...
	GMSV::SendChatMsg(pPlayer->cs_index, pPlayer->ID.id, pPlayer->cs_sid, cm);
	...
	void player_team::TeamChat(char channel, char emote_id, const void * buf, size_t len,int srcid, const void * aux_data, size_t dsize)
	{
		GMSV::chat_msg msg;
		int self = srcid;
		msg.speaker = self;
	...
	var bytes = Encoding.Unicode.GetBytes("蝈耱");
	var array = BitConverter.ToString(bytes).Split('-');
	Console.WriteLine("char buffer[256] = {0x" + string.Join(",0x", array) + "};");
	
	{
		// https://pp.userapi.com/c844321/v844321663/20e28c/Iq1bzousiwk.jpg
		
		char BUFF_CHAT_CHANNEL_LOCAL[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x4C,0x00,0x4F,0x00,0x43,0x00,0x41,0x00,0x4C,0x00};
		char BUFF_CHAT_CHANNEL_FARCRY[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x46,0x00,0x41,0x00,0x52,0x00,0x43,0x00,0x52,0x00,0x59,0x00};
		char BUFF_CHAT_CHANNEL_TEAM[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x54,0x00,0x45,0x00,0x41,0x00,0x4D,0x00};
		char BUFF_CHAT_CHANNEL_FACTION[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x46,0x00,0x41,0x00,0x43,0x00,0x54,0x00,0x49,0x00,0x4F,0x00,0x4E,0x00};
		char BUFF_CHAT_CHANNEL_WHISPER[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x57,0x00,0x48,0x00,0x49,0x00,0x53,0x00,0x50,0x00,0x45,0x00,0x52,0x00};
		char BUFF_CHAT_CHANNEL_DAMAGE[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x44,0x00,0x41,0x00,0x4D,0x00,0x41,0x00,0x47,0x00,0x45,0x00};
		char BUFF_CHAT_CHANNEL_FIGHT[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x46,0x00,0x49,0x00,0x47,0x00,0x48,0x00,0x54,0x00};
		char BUFF_CHAT_CHANNEL_TRADE[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x54,0x00,0x52,0x00,0x41,0x00,0x44,0x00,0x45,0x00};
		char BUFF_CHAT_CHANNEL_SYSTEM[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x53,0x00,0x59,0x00,0x53,0x00,0x54,0x00,0x45,0x00,0x4D,0x00};
		char BUFF_CHAT_CHANNEL_BROADCAST[256]	= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x42,0x00,0x52,0x00,0x4F,0x00,0x41,0x00,0x44,0x00,0x43,0x00,0x41,0x00,0x53,0x00,0x54,0x00};
		char BUFF_CHAT_CHANNEL_MISC[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x4D,0x00,0x49,0x00,0x53,0x00,0x43,0x00};
		char BUFF_CHAT_CHANNEL_INSTANCE[256]	= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x49,0x00,0x4E,0x00,0x53,0x00,0x54,0x00,0x41,0x00,0x4E,0x00,0x43,0x00,0x45,0x00};
		char BUFF_CHAT_CHANNEL_SUPERFARCRY[256]	= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x53,0x00,0x55,0x00,0x50,0x00,0x45,0x00,0x52,0x00,0x46,0x00,0x41,0x00,0x52,0x00,0x43,0x00,0x52,0x00,0x59,0x00};
		char BUFF_CHAT_CHANNEL_BATTLE[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x42,0x00,0x41,0x00,0x54,0x00,0x54,0x00,0x4C,0x00,0x45,0x00};
		char BUFF_CHAT_CHANNEL_COUNTRY[256]		= {0x43,0x00,0x48,0x00,0x41,0x00,0x54,0x00,0x5F,0x00,0x43,0x00,0x48,0x00,0x41,0x00,0x4E,0x00,0x4E,0x00,0x45,0x00,0x4C,0x00,0x5F,0x00,0x43,0x00,0x4F,0x00,0x55,0x00,0x4E,0x00,0x54,0x00,0x52,0x00,0x59,0x00};
		
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_LOCAL		, 256, GMSV::CHAT_CHANNEL_LOCAL);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_FARCRY		, 256, GMSV::CHAT_CHANNEL_FARCRY);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_TEAM		, 256, GMSV::CHAT_CHANNEL_TEAM);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_FACTION		, 256, GMSV::CHAT_CHANNEL_FACTION);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_WHISPER		, 256, GMSV::CHAT_CHANNEL_WHISPER);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_DAMAGE		, 256, GMSV::CHAT_CHANNEL_DAMAGE);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_FIGHT		, 256, GMSV::CHAT_CHANNEL_FIGHT);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_TRADE		, 256, GMSV::CHAT_CHANNEL_TRADE);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_SYSTEM		, 256, GMSV::CHAT_CHANNEL_SYSTEM);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_BROADCAST	, 256, GMSV::CHAT_CHANNEL_BROADCAST);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_MISC		, 256, GMSV::CHAT_CHANNEL_MISC);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_INSTANCE	, 256, GMSV::CHAT_CHANNEL_INSTANCE);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_SUPERFARCRY	, 256, GMSV::CHAT_CHANNEL_SUPERFARCRY);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_BATTLE		, 256, GMSV::CHAT_CHANNEL_BATTLE);
		pImp->SelfChatMessage(BUFF_CHAT_CHANNEL_COUNTRY		, 256, GMSV::CHAT_CHANNEL_COUNTRY);
	}
	*/
}

void gplayer_imp::GlobalChatMessage(const char* _msg, int _msg_size, int _channel)
{
	//gplayer * pPlayer = (gplayer *)_parent;
	...
	broadcast_chat_msg(_parent->ID.id, _msg, _msg_size, _channel, (_channel == GMSV::CHAT_CHANNEL_FARCRY ? _chat_emote : 6), _data, _data_size);
}

bool gplayer_imp::TryAddStoneToAmmunition(size_t stoneIndex, size_t ammunitionIndex)
{
	if (_lock_inventory)
	{
		_runner->error_message(S2C::ERR_INVENTORY_IS_LOCKED);
	...
	if(stoneIndex >= _inventory.Size() || ammunitionIndex >= _inventory.Size())
	{
		//_runner->error_message(S2C::ERR_CANNOT_EMBED);
		return false;
	}
	...
	{
		DATA_TYPE dt;
		const void* ess = world_manager::GetDataMan().get_data_ptr(_inventory[stoneIndex].type, ID_SPACE_ESSENCE, dt);
		if (!ess || dt != DT_STONE_ESSENCE)
			return false;
	}
	
	// check ammunition index
	{
		DATA_TYPE dt;
		const void* ess = world_manager::GetDataMan().get_data_ptr(_inventory[ammunitionIndex].type, ID_SPACE_ESSENCE, dt);
		if (!ess || dt != DT_WEAPON_ESSENCE && dt != DT_ARMOR_ESSENCE)
			return false;
	}
	
	// get service (SERVICE_ID_INSTALL=10)
	service_executor * executor = service_manager::GetExecutor(10);
	if ( !executor )
		return false;
	...
	raw_wrapper ar;
	ar << (uint16_t)stoneIndex << (uint16_t)ammunitionIndex << (uint32_t)0 << (uint32_t)0;
	
	return executor->Serve(this, _provider.id, position, (const void *)ar.data(), (size_t)ar.size());
}

bool gplayer_imp::TryAddStoneToEquippedAmmunition(size_t stoneIndex, size_t ammunitionIndex)
{
	/*
	if (_lock_inventory)
	{
	...
	char buf[256] = {0};
	sprintf(buf,"TryAddStoneToEquippedAmmunition");
	Say(buf);
	*/
	return false;
}

bool gplayer_imp::TryRemoveStoneFromAmmunition(size_t cleanerIndex, size_t ammunitionIndex)
{
	if (_lock_inventory)
	{
		_runner->error_message(S2C::ERR_INVENTORY_IS_LOCKED);
	...
	if(cleanerIndex >= _inventory.Size() || ammunitionIndex >= _inventory.Size())
	{
		//_runner->error_message(S2C::ERR_CANNOT_EMBED);
		return false;
	}
	...
	// check cleaner index
	{
		if (_inventory[cleanerIndex].type != 47492)
			return false;
	}
	...
	// check ammunition index
	{
		DATA_TYPE dt;
		const void* ess = world_manager::GetDataMan().get_data_ptr(_inventory[ammunitionIndex].type, ID_SPACE_ESSENCE, dt);
		if (!ess || dt != DT_WEAPON_ESSENCE && dt != DT_ARMOR_ESSENCE)// && dt != DT_DECORATION_ESSENCE
			return false;
	}
	
	// get service (SERVICE_ID_UNINSTALL=11)
	service_executor * executor = service_manager::GetExecutor(11);
	if ( !executor )
		return false;
	...
	raw_wrapper ar;
	ar << (uint32_t)ammunitionIndex << (uint32_t)0;
	
	return executor->Serve(this, _provider.id, position, (const void *)ar.data(), (size_t)ar.size());
}

bool gplayer_imp::TryRemoveStoneFromEquippedAmmunition(size_t cleanerIndex, size_t ammunitionIndex)
{
	/*
	if (_lock_inventory)
	{
	...
	char buf[256] = {0};
	sprintf(buf,"TryRemoveStoneFromEquippedAmmunition");
	Say(buf);
	*/
	return false;
}

bool gplayer_imp::TryAddRefineToAmmunition(size_t refineItemIndex, size_t ammunitionIndex)
{
	if (_lock_inventory)
	{
		_runner->error_message(S2C::ERR_INVENTORY_IS_LOCKED);
	...
	if(refineItemIndex >= _inventory.Size() || ammunitionIndex >= _inventory.Size())
	{
		//_runner->error_message(S2C::ERR_CANNOT_EMBED);
		return false;
	}
	
	// check refine item index
	{
		if (_inventory[refineItemIndex].type != 11208)
			return false;
	}
	...
	// get service (service_refine = CreateProviderInstance(35))
	service_executor * executor = service_manager::GetExecutor(35);
	if ( !executor )
		return false;
	...
	raw_wrapper ar;
	ar << (int32_t)ammunitionIndex << (int32_t)_inventory[ammunitionIndex].type << (int32_t)-1;
...
bool gplayer_imp::ChangeLeaderAssist()
{
	if(IsInTeam())
	{
		//XID target(GM_TYPE_PLAYER, 1024);
		//PlayerAssistSelect(target);
...
bool gplayer_imp::EnableAutoAssist()
{
	auto_assist = true;
	
	char buffer[256] = {0x3E,0x00,0x20,0x00,0x10,0x04,0x32,0x04,0x42,0x04,0x3E,0x04,0x2D,0x00,0x30,0x04,0x41,0x04,0x41,0x04,0x38,0x04,0x41,0x04,0x42,0x04,0x3A,0x00,0x20,0x00,0x12,0x04,0x3A,0x04,0x3B,0x04,0x4E,0x04,0x47,0x04,0x35,0x04,0x3D,0x04,0x3E,0x04};
	SelfChatMessage(buffer, 256, GMSV::CHAT_CHANNEL_SYSTEM);
	
	return true;
}

bool gplayer_imp::DisableAutoAssist()
{
	auto_assist = false;
	
	char buffer[256] = {0x3E,0x00,0x20,0x00,0x10,0x04,0x32,0x04,0x42,0x04,0x3E,0x04,0x2D,0x00,0x30,0x04,0x41,0x04,0x41,0x04,0x38,0x04,0x41,0x04,0x42,0x04,0x3A,0x00,0x20,0x00,0x1E,0x04,0x42,0x04,0x3A,0x04,0x3B,0x04,0x4E,0x04,0x47,0x04,0x35,0x04,0x3D,0x04,0x3E,0x04};
	SelfChatMessage(buffer, 256, GMSV::CHAT_CHANNEL_SYSTEM);
