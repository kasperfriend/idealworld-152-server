...
congregate_item::OnUse(item::LOCATION l, gactive_imp * imp, size_t count)
{
	...
	else if(ess->congregate_type == gplayer_imp::CONGREGATE_TYPE_FACTION_ALLIANCE)
	{
	 	if(!pImp->FactionAllianceCongregateRequest(ess->area[index].require_reply_level, ess->area[index].require_reply_level2, ess->area[index].require_reply_reincarnation_times))
		 	return -1;
		GLog::log(GLOG_INFO,"玩家%d使用了帮派集结令%d", pImp->_parent->ID.id, _tid);
	}
	return 1;
