...
//¼ì²éÏúÊÛÊ±¼ä¶Î(²»º¬¹ıÆÚÊ±¼äµÄ)ÊÇ·ñÓĞ³åÍ» true ÓĞ false Ã»ÓĞ£¬µ÷ÓÃÇ°Ó¦±£Ö¤Ã¿¸ösale_timeµÄ²ÎÊıÊÇÓĞĞ§µÄ
...
InitMall(netgame::mall & __mall, itemdataman & dataman, const abase::vector<MALL_ITEM_SERV> & __list)
{
	...
	for(size_t i = 0; i < __list.size(); i ++)
	{
		...
		{
			__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·%d(%d¸ö)³¬¹ıÁË¶ÑµşÉÏÏŞ%d\n",id,count,pile_limit);
			return false;
		}
		...
		bool bExpire = false;
		size_t slot_count = 0;
		int gid_array[4] = {-1,-1,-1,-1};	//lgc ±£´æµ±Ç°ÎïÆ·¹ºÂò·½Ê½ÖĞµÄ²»Í¬×éid
		size_t gid_count = 0;
		...
		int tz_adjust = wp_tz;	//gshop±à¼­Æ÷±£´æµÄÊ±¼äÊÇgmtime,sale_time¸ù¾İgsÔËĞĞµÄÊ±Çø½øĞĞµ÷Õû
#else
		int tz_adjust = - tm1.tm_gmtoff;	//gshop±à¼­Æ÷±£´æµÄÊ±¼äÊÇgmtime,sale_time¸ù¾İgsÔËĞĞµÄÊ±Çø½øĞĞµ÷Õû
#endif
		for(size_t j = 0; j < 4;  j ++)
		{
			...
			if(expire_time < 0)
			{
				__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·%d¹ıÆÚÊ±¼äĞ¡ÓÚ0\n",id);	
				return false;
			}
			...
			{
				__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·%dÏúÊÛÊ±¼ä²ÎÊı²»ÕıÈ·\n",id);	
				return false;
			}
			...
			if(group_id < 0)
			{
				__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·%d¹ºÂò·½Ê½×éidĞ¡ÓÚ0\n",id);	
				return false;
			}
			...
			{
				__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·%dÉÌÆ·×´Ì¬Ğ¡ÓÚ0\n",id);	
				return false;
			}
		...
		if(slot_count == 0)
		{
			__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·%d²»´æÔÚ¿ÉÓÃµÄÌõÄ¿Ë÷ÒıÒ%d\n", id, i);
			return false;
		}
		
		if(bExpire && pile_limit != 1)
		{
			__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·´æÔÚ¿É¶Ñµşµ«´æÔÚÓĞĞ§ÆÚµÄÎïÆ·%d£¬ÎŞ·¨½øĞĞ³õÊ¼»¯²Ù×÷ Ë÷Òı%d\n", id, i);
			return false;
		}
		
		if(bExpire && sale_time_active)
		{
			__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·%dÉè¶¨ÁË×â½èÊ±¼äµ«´æÔÚ²»ÊÇÓÀ¾ÃµÄÏúÊÛÊ±¼ä\n",id);	
			return false;
		}
		...
		if(bExpire)
		{
			...
			{
				int same_gid_slot_count = 0;	//Í¬Ò»¸ö×éÄÚ¹ºÂò·½Ê½Êı
				...
				if(isconflict)
				{
					__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·%d×éid%d¹ıÆÚÊ±¼äÏàÍ¬\n", id, gid_array[m]);
					return false;
				}
		...
		else
		{
			...
			for(size_t m=0; m<gid_count; m++)
			{
				...
				if(isconflict)
				{
					__PRINTINFO("°Ù±¦¸óÖĞÎïÆ·%d×éid%d¶àÖÖÏúÊÛÊ±¼ä³åÍ»\n", id, gid_array[m]);
					return false;
				}
			}
		}
		...
		//º«·ş²»¿ª·Ågroup,sale_time
		if(world_manager::GetWorldParam().korea_shop || world_manager::GetWorldParam().southamerican_shop)
		{
			if(group_active || sale_time_active)
			{
				__PRINTINFO("°Ù±¦¸óÖĞµÄÎïÆ·%d,º«·ş²»ÔÊĞí³öÏÖ¹ºÂò·½Ê½µÄ×é¿ØÖÆºÍÏúÊÛÊ±¼ä¿ØÖÆ", id);
				return false;				
			}
			if(node.gift_id > 0)
			{
				__PRINTINFO("°Ù±¦¸óÖĞµÄÎïÆ·%d,º«·şÔİ²»Ö§³ÖÂòÒ»ÔùÒ»\n", id);	
				return false;
			}
		}
		//Ôİ²»¿ª·Å×é¿ØÖÆ¹¦ÄÜ
		if(group_active)
		{
				__PRINTINFO("°Ù±¦¸óÖĞµÄÎïÆ·%d,Ôİ²»¿ª·Å¹ºÂò·½Ê½µÄ×é¿ØÖÆ", id);
				return false;				
		}
		...
		//__PRINTINFO("¼ÓÈë%d¸ö%dµ½°×±¦¸óÖĞ\n",count,id);
	}
	__PRINTINFO("°Ù±¦¸óÄÚ¹²ÓĞ%dÑù»õÎï\n",__list.size());
...
InitTouchShop(netgame::touchshop & __shop, itemdataman & dataman)
{
	...
	for(int pi = 0; pi < TSC_PAGE_MAX; ++pi)
	{
		for(int i = 0; i < TSC_PAGE_ITEM_MAX; ++i)
		{
			...
			{
				printf("touch shop ´æÔÚ·Ç·¨ÊıÁ¿%dpage%dindex%d\n",count,pi,i);
