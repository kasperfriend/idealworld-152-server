...
int service_npc::_pepega_npc_wid = -1;
...
service_npc::MessageHandler(world * pPlane ,const MSG & msg)
{
	switch(msg.message)
	{
		...
		{
			printf("[__PEPEGA__][GM_MSG_SERVICE_REQUEST] use service executor [id=%d]\n", msg.param);
			
			if(!NPCService::IsRemoteService(msg.param) && !CheckDistanceLimit(msg.pos))
			{
...
{
	int tepmlateID = GetNPCID();
	
	if(_serve_distance_unlimited) _plane->AddSceneServiceNpc(tepmlateID, _parent->ID.id);
	
	if ( tepmlateID == 59885 )
	{
		service_npc::SetPepegaNPC(_parent->ID.id);
		printf("[__PEPEGA__][TurboMegaNPC3000][Enter] WID = %d\n", _parent->ID.id); //  pObject->pos
	}
}
...
service_npc::OnNpcLeaveWorld()
{
	...
	if(_serve_distance_unlimited) _plane->DelSceneServiceNpc(tepmlateID, _parent->ID.id);
	
	if ( tepmlateID == 59885 )
	{
		service_npc::SetPepegaNPC(-1);
		printf("[__PEPEGA__][TurboMegaNPC3000][Leave] WID = %d\n", _parent->ID.id); //  pObject->pos
	}
}
...
void 
service_npc::OnHeartbeat(size_t tick)
{
