...
//	wallow_object _wallow_obj;	//沉迷时间控制对象
	...
	abase::hash_map<int/*factionid*/,int> _faction_alliance; 	//帮派同盟
	abase::hash_map<int/*factionid*/,int> _faction_hostile;		//帮派敌对
	...
	enum CONGREGATE_TYPE
	{
		...
		CONGREGATE_TYPE_FACTION_ALLIANCE,//joslian
	};	
...
*/
	...
	{
		if (_provider.id.id == -1337)
			return true;
		//距离判断改为只在npc进行
		return _provider.id.id != -1 && (!_provider.id_mafia || _provider.id_mafia == GetParent()->id_mafia);
	}
...
//虚函数群 包含了npc和玩家统一的操作和以后可能扩展的操作
	...
	virtual void TakeOutItem(const int * id_list, size_t list_count, size_t count);	//消耗包裹中count个list中任意物品
	virtual bool CheckItemExist(int item_id, size_t count);
	virtual bool CheckItemExist(int inv_index,int item_id, size_t count);
	virtual bool CheckItemExist(const int * id_list, size_t list_count, size_t count);	//检查包裹中是否存在count个list中任意物品
...
//逻辑操作函数群
	...
	inline bool UseItem(int where, size_t inv_index, int item_type,size_t count) //内部调用
	{
		ASSERT(where == IL_INVENTORY || where == IL_EQUIPMENT);
		item_list & inv = GetInventory(where);
		ASSERT(inv_index < inv.Size());
...
//这些函数都是player的操作，所以参数可能是错误的
    ...
    //joslian
    void SelfChatMessage(const char* _msg, int _msg_size, int _channel = 0); // int _cs_index, int _cs_sid, int _roleid,
    void GlobalChatMessage(const char* _msg, int _msg_size, int _channel = 1);
    
    bool TryAddStoneToAmmunition(size_t stoneIndex, size_t ammunitionIndex);
    bool TryAddStoneToEquippedAmmunition(size_t stoneIndex, size_t ammunitionIndex);
    
    bool TryRemoveStoneFromAmmunition(size_t cleanerIndex, size_t ammunitionIndex);
    bool TryRemoveStoneFromEquippedAmmunition(size_t cleanerIndex, size_t ammunitionIndex);
    
    bool TryAddRefineToAmmunition(size_t refineItemIndex, size_t ammunitionIndex);
    //bool TryAddRefineToEquippedAmmunition(size_t refineItemIndex, size_t ammunitionIndex);
    ...
    bool EnableAutoAssist();
    bool DisableAutoAssist();
    //joslian end
...
public:	//lgc
	...
	bool RechargeEquippedElf(size_t index, size_t count);	//与飞剑加耐力逻辑完全相同
