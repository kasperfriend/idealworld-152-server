...
private:
	...
	virtual bool SendRequest(gplayer_imp * pImp, const XID & provider,const void * buf, size_t size)
	{
		...
		{
			A3DVECTOR position;
			return OnServe(pImp, provider, position, buf, size);
		}*/
		//joslian end
...
public:
	struct player_request
	{
		size_t pet_index;		//要染色的宠物在宠物栏的位置
		int pet_tid;		//宠物的模板id
		size_t dye_index;		//染色材料的位置 
		int dye_type;		//染色剂的id
	};
private:
...
private:
	...
	virtual bool SendRequest(gplayer_imp * pImp, const XID & provider,const void * buf, size_t size)
	{
		printf("[__PEPEGA__][country_service_executor][SendRequest] __1__\n");
		if(size != sizeof(player_request)) return false;
		player_request * req = (player_request *)buf;
		
		printf("[__PEPEGA__][country_service_executor][SendRequest] __2__ type: %d\n", req->type);
	...
	virtual bool OnServe(gplayer_imp *pImp,const XID & provider, const A3DVECTOR & pos,const void * buf, size_t size)
	{
		printf("[__PEPEGA__][country_service_executor][OnServe] __1__\n");
		ASSERT(size == sizeof(player_request));
		player_request * req = (player_request*)buf;
		
		printf("[__PEPEGA__][country_service_executor][OnServe] __2__ type: %d\n", req->type);
