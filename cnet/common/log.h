...
namespace GNET
{
	...
	public:
		static const char * prioritystring( int __priority )
		{
			...
			static const char * sz_money   = "money_change";
	...
	public:
		...
		static void login( Octets account, int userid, unsigned int sid, const std::string & peer, const std::string & mac, const std::string & mid )
		{
			...
			memcpy( buf, (char*)account.begin(), len );
			buf[len] = 0;
			logvital( LOG_NOTICE, "formatlog:login:account=%s:userid=%d:sid=%u:peer=%s:mac=%s:mid=%s",
							buf, userid, sid, peer.c_str(), mac.c_str(), mid.c_str() );
		}
