...
struct SessionInfo
{
	...
	char	passwd_flag;	//bit0:ÃÜÂëÌ«¾ÃÎ´¸ü»» bit1:ÃÜ±£¿¨Ì«¾ÃÎ´¸ü»»
	char	accountinfo_flag;//bit0:ÐèÒª²¹ÌîÉí·ÝÖ¤ºÍÐÕÃû bit1:ÐèÒª²¹ÌîÕËºÅÃÜÂëÓÊÏä
	Octets used_elec_number; //±¾´ÎµÇÂ½Ê¹ÓÃµÄ¶þ´úÉñ¶Üµç×ÓÂë
	Octets cli_fingerprint; //¼ÇÂ¼ÁË°üÀ¨¿Í»§¶Ë°æ±¾ÔÚÄÚµÄ²¿·ÖÐÅÏ¢
	...
	bool IsOnlineSave;
    uint64_t m_MachineHash;
    bool m_IsWindowRegistered;
    bool save_logout;
    bool m_ClientCheck;
    
	SessionInfo() : userid(0), roleid(0), gsid(0), login_time(0), busy(0), local(0), peer(0), checker(NULL), passwd_flag(0), accountinfo_flag(0),
		IsOnlineSave(false), m_MachineHash(0L), m_IsWindowRegistered(false), m_ClientCheck(false)
...
public:
	...
	/*switch user map*/
	typedef std::map<Session::ID, RoleData>		SwitchUserMap;
	typedef std::deque<Protocol*> ProtocolQueue;
	typedef std::map<Session::ID, ProtocolQueue>	AccumulateProtocolMap;
	...
	typedef std::map<uint64_t, uint32_t>	WindowPog;
    typedef std::map<uint32_t, WindowPog>	WindowsMap;
    WindowsMap m_WindowsMap;
    
    Thread::Mutex m_WindowsMapMutex;
	...
	{ 
		...
		// apfilipp
	    m_WindowsMap[58][0] = 0; // ID=58, Tag=128, is28 - a28 - Äîëèíà áåçäóøèÿ(Ãóé)
	    m_WindowsMap[75][0] = 0; // ID=75, Tag=135, is35 - a35 - Áàçà ãèëüäèè(Êëàí õîëë)
	}
	...
	typedef std::map<int, RoleData>	RoleMap;
	...
	typedef std::map<Session::ID, int> AlivetimeMap;
	typedef std::map<Session::ID, int> ReadyCloseMap;
	AlivetimeMap alivetimemap;
	ReadyCloseMap readyclosemap;

	//forbid chat timer map	
	typedef std::map<int/*userid*/,GRoleForbid> MuteMap;
	...
	//user privilege map
	typedef std::map<int /*id*/,ByteVector>  PrivilegeMap;
