...
int CALC_PHYSIC_DMG(object_interface& object,Skill* skill,int pdmg, int ndmg)
{
	...
	if(object.IsPlayerClass())
	{
		...
		if(skill->GetPerformerid().IsPlayerClass())	damage = damage >> 2;
	}
	else
	{
...
void PlayerWrapper::SetAddball(int type)
{
	...
	{
		if(oldvstate) object.DecVisibleState(oldvstate); 
		if(newvstate) object.IncVisibleState(newvstate); 
...
bool PlayerWrapper::SetPerform(int inform)
{
	...
	if(inform==2)
	{
		...
		if(skill->GetStub()->cls != 258){	//不是小精灵技能才会扣ap,小精灵技能需扣小精灵的体力，在skillstub中实现
	...
	if(skill->GetStub()->GetType()==TYPE_ATTACK)
	{
		...
		case Range::LINE:
			if(success)
			{
				A3DVECTOR tpos;
				float tbody;
				if(!target || 1!=object.QueryObject(*target, tpos, tbody)
						|| object.GetPos().squared_distance(tpos)<1e-6)
				{
					success = false;
				}
				else
		...
		case Range::TARGETBALL:
			if(success)
			{
				...
				float correction;
				if(target && 1==object.QueryObject(*target, tpos, tbody) 
						&& GetInrange( tpos, tbody, skill->GetEffectdistance(), correction))
				{
					msg.attack_range		= skill->GetEffectdistance();
					if( msg.attack_range <= 0 ) msg.attack_range	= object.GetExtendProp().attack_range;
					msg.attack_range += correction;
			...
			if(stype==TYPE_BLESS || stype==TYPE_NEUTRALBLESS || stype==TYPE_NEUTRALBLESS2)
			{
				if(object.IsPet())
				{
					...
					}else
					{
						...
						A3DVECTOR tpos;
						float tbody;
						if(object.QueryObject(object.GetLeaderID(),tpos,tbody)==1 
								&& tpos.squared_distance(object.GetPos()) < msg.attack_range*msg.attack_range)
						{
...
bool PlayerWrapper::CheckTarget(float range, char type)
{
	...
	{
		if(stype!=TYPE_BLESS && stype!=TYPE_NEUTRALBLESS && stype!=TYPE_NEUTRALBLESS2) return false;
	}
...
bool PlayerWrapper::SetScaledechp(bool)
{
	if(ThrowDice())
	{
		...
		{
			if(skill->GetAttackState() & 0x4000 && !object.IsPlayerClass()) return false;
...
bool PlayerWrapper::SetSkillcooldown(bool)
{
	if(ThrowDice())
	{
		if(value > 0) object.SetCoolDown(GetValueInt()+COOLINGID_BEGIN, GetAmountInt());
		if(ratio > 0) object.SetCoolDown(GetRatioInt()+COOLINGID_BEGIN, GetAmountInt());
	}
	return true;
}
...
//		value = object.CalcVigourEnhanceDamage(skill->GetVigour(),	object.GetVigour(),	IS_PVP(object,skill), GetValueInt());
		...
		filter_Aurafireattack * pfilter 
			= new filter_Aurafireattack(object,time,(int)value,ratio);
...
bool PlayerWrapper::SetAurabless(bool)	
{
	if(ThrowDice())
	{
		filter_Aurabless * pfilter 
			= new filter_Aurabless(object,time,ratio,GetValueInt(),GetAmountInt());
		pfilter->SetTalent(skill->GetT0(),skill->GetT1(),skill->GetT2());
		object.AddFilter(pfilter);
	}
	return true;
...
bool PlayerWrapper::SetAuracurse(bool)
{
	if(ThrowDice())
	{
		filter_Auracurse * pfilter 
			= new filter_Auracurse(object,time,ratio,GetValueInt(),GetAmountInt());
		pfilter->SetTalent(skill->GetT0(),skill->GetT1(),skill->GetT2());
		object.AddFilter(pfilter);
	}
	return true;
...
{
	filter_Aurabless * pfilter 
		= new filter_Aurabless(object,time,ratio,GetValueInt(),GetAmountInt(),(int)(probability+0.1f));
	pfilter->SetTalent(skill->GetT0(),skill->GetT1(),skill->GetT2());
	object.AddFilter(pfilter);
...
{
	filter_Auracurse * pfilter 
		= new filter_Auracurse(object,time,ratio,GetValueInt(),GetAmountInt(),(int)(probability+0.1f));
	pfilter->SetTalent(skill->GetT0(),skill->GetT1(),skill->GetT2());
	object.AddFilter(pfilter);
...
{
	char delete_type = 0;
	if(amount > 1.5f) delete_type = 2;
	else if(amount > 0.5f) delete_type = 1;
...
bool PlayerWrapper::SetSpecialphysichurt(bool)
{
	...
	if(object.IsFilterExist(FILTER_SPECIALPHYSICHURTTRIGGER)) damage = (int)(damage*(1.f+ratio));
	if(damage > 0)	
	{
		object.BeHurt(skill->GetPerformerid(),skill->GetPerformerinfo(), damage, invader, 
				skill->GetAttackerMode());	
...
bool PlayerWrapper::SetPalsy(bool b)
{
	if(ThrowDice())
	{
		...
		{ 
			...
			int mask = (value > 1.0f ? filter::FILTER_MASK_DEBUFF : (value > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
...
bool PlayerWrapper::SetAddfrosteffect(bool)
{
    ...
    if(object.GetImmuneMask() & IMMUNEMETAL) metal_dmg = 0;
    if(object.GetImmuneMask() & IMMUNEWATER) water_dmg = 0;
...
{
    if (!ThrowDice()) return false;
    if (!object.IsFilterExist(FILTER_ADDFROSTEFFECT)) return false;
